export class Processor {
  ...
  printRegisters(): string[] {
    ...
    registerString.push('Accumulator:           ' + this.formatNumber(5, this.accumulator));
    registerString.push('InstructionCounter:       ' + this.formatNumber(2, this.instructionCounter));
    registerString.push('InstructionRegister:   ' + this.formatNumber(5, this.instructionRegister));
    registerString.push('OperationCode:            ' + this.formatNumber(2, this.opCode));
  ...
  printMemory(): string[][] {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (let j = 0; j < 10; j++) {
        memoryString[i + 1].push(this.formatNumber(5, this.memory[(i * 10) + j]));
      }
    }
